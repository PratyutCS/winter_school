          ...
          <div className="flex flex-wrap items-center justify-center w-full gap-x-4 gap-y-2">
            {navLinks.map((link, i) => (
              <Link
                key={i}
                to={link.path}
                className={`relative group px-3 py-2.5 transition-all duration-300 animate-fade-in ${location.pathname === link.path ? 'active-nav-link' : ''
                  }`}
                style={{ animationDelay: `${i * 50}ms` }}
              >
                <span className={`relative z-10 font-medium text-[#2e2a30] text-sm tracking-wide
                               group-hover:text-[#7c3aed] transition-colors duration-300 ${location.pathname === link.path ? 'text-[#7c3aed]' : ''
                  }`}>
                  {link.label}
                </span>
                <span className={`absolute left-0 -bottom-1 w-0 h-0.5 bg-gradient-to-r
                               from-[#7c3aed] to-[#ec4899] rounded-full
                               transition-all duration-300 group-hover:w-full ${location.pathname === link.path ? 'w-full' : ''
                  }`}></span>
                <span className={`absolute inset-0 -z-10 rounded-xl opacity-0
                               group-hover:opacity-100 transition-opacity duration-300
                               bg-[#7c3aed]/10 backdrop-blur-sm ${location.pathname === link.path ? 'opacity-100' : ''
                  }`}></span>
              </Link>
            ))}
          </div>
            ...
            {navLinks.map((link, i) => (
              <Link
                ...
                className={`block py-3 px-6 text-[#2e2a30] hover:text-[#7c3aed]
                         ...
                         text-sm font-medium animate-fade-in ${location.pathname === link.path ? 'text-[#7c3aed] bg-[#7c3aed]/10' : ''
                  }`}
                style={{ animationDelay: `${i * 50}ms` }}
